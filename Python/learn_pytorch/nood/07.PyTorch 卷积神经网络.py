...
# --- 这里是关键修改！将用于可视化的数据也移动到 GPU ---
images = images.to(device)
labels = labels.to(device) # 虽然标签不需要参与模型计算，但如果后续与预测结果一同处理，放在同一设备更安全

# 在GPU上执行模型推理
with torch.no_grad(): # 确保在推理时关闭梯度计算
    outputs = model(images)
...
for i in range(6):
    ...
    axes[i].set_title(f"Label: {labels[i].cpu().item()}\nPred: {predictions[i].cpu().item()}")
    axes[i].axis('off')
plt.show()